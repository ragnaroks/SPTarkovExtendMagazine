...
const baseId: string = '68c437db3385046e4fd37100';
...
export default function addExtendMagazine_VSK94(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_KBP_VSK94_9X39_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VSK94，' + ItemTpl.ASSAULTCARBINE_KBP_VSK94_9X39_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X39_9A91_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VSK94，' + ItemTpl.MAGAZINE_9X39_9A91_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'VSK-94 extended magazine',
        shortName: 'Extended',
        description: 'VSK-94 extended magazine'
      },
      ch: {
        name: 'VSK-94 扩容弹匣',
        shortName: '扩容',
        description: 'VSK-94 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VSK94，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_VSK94，Id：' + createResult.itemId);
}
