...
const baseId: string = '68c43339362153620fa7eb00';
...
export default function addExtendMagazine_MCXSPEAR(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_SIG_MCXSPEAR_68X51_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MCXSPEAR，' + ItemTpl.ASSAULTRIFLE_SIG_MCXSPEAR_68X51_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_L7AWM_25RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MCXSPEAR，' + ItemTpl.MAGAZINE_762X51_L7AWM_25RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MCX SPEAR extended magazine',
        shortName: 'Extended',
        description: 'MCX SPEAR extended magazine'
      },
      ch: {
        name: 'MCX SPEAR 扩容弹匣',
        shortName: '扩容',
        description: 'MCX SPEAR 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MCXSPEAR，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MCXSPEAR，Id：' + createResult.itemId);
}
