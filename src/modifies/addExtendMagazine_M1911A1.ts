...
const baseId: string = '68c45f2447f4cf9a61552d00';
...
export default function addExtendMagazine_M1911A1(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_COLT_M1911A1_45_ACP] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M1911A1，' + ItemTpl.PISTOL_COLT_M1911A1_45_ACP + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_45ACP_1911_7RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M1911A1，' + ItemTpl.MAGAZINE_45ACP_1911_7RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M1911A1 extended magazine',
        shortName: 'Extended',
        description: 'M1911A1 extended magazine'
      },
      ch: {
        name: 'M1911A1 扩容弹匣',
        shortName: '扩容',
        description: 'M1911A1 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M1911A1，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M1911A1，Id：' + createResult.itemId);
}
