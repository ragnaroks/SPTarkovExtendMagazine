...
const baseId: string = '68c44f5f359d193a8c93f600';
...
export default function addExtendMagazine_SAIGA12K(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_SAIGA12K_VER10_12GA_SEMIAUTOMATIC] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA12K，' + ItemTpl.SHOTGUN_SAIGA12K_VER10_12GA_SEMIAUTOMATIC + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_12G_SB5_5RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA12K，' + ItemTpl.MAGAZINE_12G_SB5_5RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Saiga-12K extended magazine',
        shortName: 'Extended',
        description: 'Saiga-12K extended magazine'
      },
      ch: {
        name: 'Saiga-12K 扩容弹匣',
        shortName: '扩容',
        description: 'Saiga-12K 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA12K，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA12K，Id：' + createResult.itemId);
}
