...
const baseId: string = '68c424681a264a79ee051700';
...
export default function addExtendMagazine_AK74(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK74_545X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK74，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK74_545X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_6L20_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK74，' + ItemTpl.MAGAZINE_545X39_6L20_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AK-74 extended magazine',
        shortName: 'Extended',
        description: 'AK-74 extended magazine'
      },
      ch: {
        name: 'AK-74 扩容弹匣',
        shortName: '扩容',
        description: 'AK-74 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK74，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AK74，Id：' + createResult.itemId);
}
