...
const baseId: string = '68c45f7a0775f4e8a74c9200';
...
export default function addExtendMagazine_MP443(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_YARYGIN_MP443_GRACH_9X19] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP443，' + ItemTpl.PISTOL_YARYGIN_MP443_GRACH_9X19 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MP443_18RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP443，' + ItemTpl.MAGAZINE_9X19_MP443_18RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP-443 extended magazine',
        shortName: 'Extended',
        description: 'MP-443 extended magazine'
      },
      ch: {
        name: 'MP-443 扩容弹匣',
        shortName: '扩容',
        description: 'MP-443 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP443，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP443，Id：' + createResult.itemId);
}
