...
const baseId: string = '68c44c81fae4f1263c134500';
...
export default function addExtendMagazine_M590A1(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_MOSSBERG_590A1_12GA_PUMPACTION] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M590A1，' + ItemTpl.SHOTGUN_MOSSBERG_590A1_12GA_PUMPACTION + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_12G_590A1X8_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M590A1，' + ItemTpl.MAGAZINE_12G_590A1X8_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M590A1 extended magazine',
        shortName: 'Extended',
        description: 'M590A1 extended magazine'
      },
      ch: {
        name: 'M590A1 扩容弹匣',
        shortName: '扩容',
        description: 'M590A1 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M590A1，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M590A1，Id：' + createResult.itemId);
}
