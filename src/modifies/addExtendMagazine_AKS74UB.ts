...
const baseId: string = '68c42da7d01ec137cd7cc200';
...
export default function addExtendMagazine_AKS74UB(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKS74UB_545X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UB，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKS74UB_545X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_6L20_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UB，' + ItemTpl.MAGAZINE_545X39_6L20_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AKS-74UB extended magazine',
        shortName: 'Extended',
        description: 'AKS-74UB extended magazine'
      },
      ch: {
        name: 'AKS-74UB 扩容弹匣',
        shortName: '扩容',
        description: 'AKS-74UB 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UB，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UB，Id：' + createResult.itemId);
}
