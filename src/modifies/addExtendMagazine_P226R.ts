...
const baseId: string = '68c45fb2db5b9ffd80bae500';
...
export default function addExtendMagazine_P226R(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_SIG_P226R_9X19] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P226R，' + ItemTpl.PISTOL_SIG_P226R_9X19 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_P226_15RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P226R，' + ItemTpl.MAGAZINE_9X19_P226_15RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'P226R extended magazine',
        shortName: 'Extended',
        description: 'P226R extended magazine'
      },
      ch: {
        name: 'P226R 扩容弹匣',
        shortName: '扩容',
        description: 'P226R 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P226R，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_P226R，Id：' + createResult.itemId);
}
