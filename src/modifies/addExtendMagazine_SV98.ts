...
const baseId: string = '68c43b42297ef1b9ee571d00';
...
export default function addExtendMagazine_SV98(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SNIPERRIFLE_SV98_762X54R_BOLTACTION_SNIPER_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SV98，' + ItemTpl.SNIPERRIFLE_SV98_762X54R_BOLTACTION_SNIPER_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X54R_SV98_10RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SV98，' + ItemTpl.MAGAZINE_762X54R_SV98_10RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SV-98 extended magazine',
        shortName: 'Extended',
        description: 'SV-98 extended magazine'
      },
      ch: {
        name: 'SV-98 扩容弹匣',
        shortName: '扩容',
        description: 'SV-98 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SV98，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SV98，Id：' + createResult.itemId);
}
