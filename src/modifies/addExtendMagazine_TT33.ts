...
const baseId: string = '68c461aa4cc176548ff49300';
...
export default function addExtendMagazine_TT33(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_TT33_762X25_TT] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33，' + ItemTpl.PISTOL_TT33_762X25_TT + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X25TT_TT105_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33，' + ItemTpl.MAGAZINE_762X25TT_TT105_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'TT-33 extended magazine',
        shortName: 'Extended',
        description: 'TT-33 extended magazine'
      },
      ch: {
        name: 'TT-33 扩容弹匣',
        shortName: '扩容',
        description: 'TT-33 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_TT33，Id：' + createResult.itemId);
}
