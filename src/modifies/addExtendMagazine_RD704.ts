...
const baseId: string = '68c433f7c340ecf423af6700';
...
export default function addExtendMagazine_RD704(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  ...
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RD704，' + ItemTpl.MAGAZINE_366TKM_AK55_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'RD-704 extended magazine',
        shortName: 'Extended',
        description: 'RD-704 extended magazine'
      },
      ch: {
        name: 'RD-704 扩容弹匣',
        shortName: '扩容',
        description: 'RD-704 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RD704，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_RD704，Id：' + createResult.itemId);
}
