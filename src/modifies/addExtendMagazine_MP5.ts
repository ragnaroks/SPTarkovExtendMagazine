...
const baseId: string = '68c4512ade8d8896cf8c0000';
...
export default function addExtendMagazine_MP5(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_HK_MP5_9X19_SUBMACHINE_GUN_NAVY_3_ROUND_BURST] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5，' + ItemTpl.SMG_HK_MP5_9X19_SUBMACHINE_GUN_NAVY_3_ROUND_BURST + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MP5_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5，' + ItemTpl.MAGAZINE_9X19_MP5_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        ...
        shortName: 'Extended',
        description: 'MP5 extended magazine'
      },
      ch: {
        name: 'MP5 扩容弹匣',
        shortName: '扩容',
        description: 'MP5 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP5，Id：' + createResult.itemId);
}
