...
const baseId: string = '68c454369d9d0cfd74ab0600';
...
export default function addExtendMagazine_PP9(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_PP9_KLIN_9X18PMM_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9，' + ItemTpl.SMG_PP9_KLIN_9X18PMM_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_PP91_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9，' + ItemTpl.MAGAZINE_9X18PM_PP91_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PP-9 Klin extended magazine',
        shortName: 'Extended',
        description: 'PP-9 Klin extended magazine'
      },
      ch: {
        name: 'PP-9 Klin 扩容弹匣',
        shortName: '扩容',
        description: 'PP-9 Klin 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PP9，Id：' + createResult.itemId);
}
