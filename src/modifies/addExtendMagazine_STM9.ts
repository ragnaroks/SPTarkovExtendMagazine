...
const baseId: string = '68c456b3673007b70a0d7300';
...
export default function addExtendMagazine_STM9(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_SOYUZTM_STM9_GEN2_9X19_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_STM9，' + ItemTpl.SMG_SOYUZTM_STM9_GEN2_9X19_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_BIG_STICK_33RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_STM9，' + ItemTpl.MAGAZINE_9X19_BIG_STICK_33RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'STM-9 extended magazine',
        shortName: 'Extended',
        description: 'STM-9 extended magazine'
      },
      ch: {
        name: 'STM-9 扩容弹匣',
        shortName: '扩容',
        description: 'STM-9 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_STM9，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_STM9，Id：' + createResult.itemId);
}
