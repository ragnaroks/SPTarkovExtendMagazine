...
const baseId: string = '68c40900a0894adf988a8d00';
...
export default function addExtendMagazine_9A91(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_KBP_9A91_9X39_COMPACT_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_9A91，' + ItemTpl.ASSAULTCARBINE_KBP_9A91_9X39_COMPACT_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X39_9A91_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_9A91，' + ItemTpl.MAGAZINE_9X39_9A91_20RND + ' not found');
    return;
  }
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_9A91，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_9A91，Id：' + createResult.itemId);
}
