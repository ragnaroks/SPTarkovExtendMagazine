...
const baseId: string = '68c420b42c1ecc67a4ca8200';
...
export default function addExtendMagazine_VPO101(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_MOLOT_ARMS_VPO101_VEPRHUNTER_762X51_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO101，' + ItemTpl.ASSAULTCARBINE_MOLOT_ARMS_VPO101_VEPRHUNTER_762X51_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_VPO101_10RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO101，' + ItemTpl.MAGAZINE_762X51_VPO101_10RND + ' not found');
    return;
  }
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO101，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_VPO101，Id：' + createResult.itemId);
}
