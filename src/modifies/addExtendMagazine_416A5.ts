...
const baseId: string = '68c430f1e73a45c2666bfd00';
...
export default function addExtendMagazine_416A5(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_HK_416A5_556X45_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_416A5，' + ItemTpl.ASSAULTRIFLE_HK_416A5_556X45_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_HK_STEEL_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_416A5，' + ItemTpl.MAGAZINE_556X45_HK_STEEL_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: '416A5 extended magazine',
        shortName: 'Extended',
        description: '416A5 extended magazine'
      },
      ch: {
        name: '416A5 扩容弹匣',
        shortName: '扩容',
        description: '416A5 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_416A5，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_416A5，Id：' + createResult.itemId);
}
