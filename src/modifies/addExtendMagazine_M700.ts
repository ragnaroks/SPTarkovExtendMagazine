...
export default function addExtendMagazine_M700(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  ...
  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_AICS_10RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M700，' + ItemTpl.MAGAZINE_762X51_AICS_10RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M700 extended magazine',
        shortName: 'Extended',
        description: 'M700 extended magazine'
      },
      ch: {
        name: 'M700 扩容弹匣',
        shortName: '扩容',
        description: 'M700 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M700，Id：' + createResult.itemId);
}
