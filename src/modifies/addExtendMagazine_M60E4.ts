...
const baseId: string = '68c446f7cd7c2951c2a30e00';
...
export default function addExtendMagazine_M60E4(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_US_ORDNANCE_M60E4_762X51_LIGHT_MACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E4，' + ItemTpl.MACHINEGUN_US_ORDNANCE_M60E4_762X51_LIGHT_MACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E4，' + ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M60E4 extended magazine',
        shortName: 'Extended',
        description: 'M60E4 extended magazine'
      },
      ch: {
        name: 'M60E4 扩容弹匣',
        shortName: '扩容',
        description: 'M60E4 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E4，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M60E4，Id：' + createResult.itemId);
}
