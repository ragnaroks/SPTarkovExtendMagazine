...
const baseId: string = '68c44738800cab21b800fd00';
...
export default function addExtendMagazine_M60E6(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_US_ORDNANCE_M60E6_762X51_LIGHT_MACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6，' + ItemTpl.MACHINEGUN_US_ORDNANCE_M60E6_762X51_LIGHT_MACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6，' + ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M60E6 extended magazine',
        shortName: 'Extended',
        description: 'M60E6 extended magazine'
      },
      ch: {
        name: 'M60E6 扩容弹匣',
        shortName: '扩容',
        description: 'M60E6 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6，Id：' + createResult.itemId);
}
