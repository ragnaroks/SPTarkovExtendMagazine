...
const baseId: string = '68c44adcf4d46bb6f4368700';
...
export default function addExtendMagazine_AA12G2(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_MPS_AUTO_ASSAULT12_GEN_2_12GA_AUTOMATIC] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AA12G2，' + ItemTpl.SHOTGUN_MPS_AUTO_ASSAULT12_GEN_2_12GA_AUTOMATIC + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_12G_AA12_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AA12G2，' + ItemTpl.MAGAZINE_12G_AA12_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AA12 Gen 2 extended magazine',
        shortName: 'Extended',
        description: 'AA12 Gen 2 extended magazine'
      },
      ch: {
        name: 'AA12 Gen 2 扩容弹匣',
        shortName: '扩容',
        description: 'AA12 Gen 2 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AA12G2，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AA12G2，Id：' + createResult.itemId);
}
