...
const baseId: string = '68c4586f46752181db8b7400';
...
export default function addExtendMagazine_UZIPS(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_IWI_UZI_PRO_SMG_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPS，' + ItemTpl.SMG_IWI_UZI_PRO_SMG_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_UZI_PRO_32RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPS，' + ItemTpl.MAGAZINE_9X19_UZI_PRO_32RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'UZI PRO SMG extended magazine',
        shortName: 'Extended',
        description: 'UZI PRO SMG extended magazine'
      },
      ch: {
        name: 'UZI PRO SMG 扩容弹匣',
        shortName: '扩容',
        description: 'UZI PRO SMG 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPS，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPS，Id：' + createResult.itemId);
}
