...
const baseId: string = '68c42a660109bae0441aed00';
...
export default function addExtendMagazine_AK104(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK104_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK104，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK104_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_AK103_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK104，' + ItemTpl.MAGAZINE_366TKM_AK103_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AK-104 extended magazine',
        shortName: 'Extended',
        description: 'AK-104 extended magazine'
      },
      ch: {
        name: 'AK-104 扩容弹匣',
        shortName: '扩容',
        description: 'AK-104 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK104，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AK104，Id：' + createResult.itemId);
}
