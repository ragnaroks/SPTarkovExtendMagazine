...
const baseId: string = '68c42dd0c382a5f39201c600';
...
export default function addExtendMagazine_AKS74UN(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKS74UN_545X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UN，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKS74UN_545X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_6L20_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UN，' + ItemTpl.MAGAZINE_545X39_6L20_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AKS-74UN extended magazine',
        shortName: 'Extended',
        description: 'AKS-74UN extended magazine'
      },
      ch: {
        name: 'AKS-74UN 扩容弹匣',
        shortName: '扩容',
        description: 'AKS-74UN 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UN，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AKS74UN，Id：' + createResult.itemId);
}
