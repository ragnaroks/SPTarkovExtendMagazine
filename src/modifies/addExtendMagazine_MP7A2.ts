...
const baseId: string = '68c4528b9d2f695022bb3900';
...
export default function addExtendMagazine_MP7A2(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_HK_MP7A2_46X30_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP7A2，' + ItemTpl.SMG_HK_MP7A2_46X30_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_46X30_MP7_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP7A2，' + ItemTpl.MAGAZINE_46X30_MP7_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP7A2 extended magazine',
        shortName: 'Extended',
        description: 'MP7A2 extended magazine'
      },
      ch: {
        name: 'MP7A2 扩容弹匣',
        shortName: '扩容',
        description: 'MP7A2 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP7A2，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP7A2，Id：' + createResult.itemId);
}
