...
const baseId: string = '68c458c4f43d65638042cf00';
...
export default function addExtendMagazine_Vector45(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_TDI_KRISS_VECTOR_GEN2_45_ACP_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector45，' + ItemTpl.SMG_TDI_KRISS_VECTOR_GEN2_45_ACP_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_45ACP_G30_MAGEX_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector45，' + ItemTpl.MAGAZINE_45ACP_G30_MAGEX_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Vector .45 extended magazine',
        shortName: 'Extended',
        description: 'Vector .45 extended magazine'
      },
      ch: {
        name: 'Vector .45 扩容弹匣',
        shortName: '扩容',
        description: 'Vector .45 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector45，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_Vector45，Id：' + createResult.itemId);
}
