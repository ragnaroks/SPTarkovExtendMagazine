...
const baseId: string = '68c45621cdd155e58ee89100';
...
export default function addExtendMagazine_SAIGA9(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_SAIGA9_9X19_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA9，' + ItemTpl.SMG_SAIGA9_9X19_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_PP1901_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA9，' + ItemTpl.MAGAZINE_9X19_PP1901_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Saiga-9 extended magazine',
        shortName: 'Extended',
        description: 'Saiga-9 extended magazine'
      },
      ch: {
        name: 'Saiga-9 扩容弹匣',
        shortName: '扩容',
        description: 'Saiga-9 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA9，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SAIGA9，Id：' + createResult.itemId);
}
