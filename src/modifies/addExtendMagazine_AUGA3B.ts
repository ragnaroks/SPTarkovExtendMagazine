...
const baseId: string = '68c42f879d06c6c60bd0e100';
...
export default function addExtendMagazine_AUGA3B(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_STEYR_AUG_A3_556X45_ASSAULT_RIFLE_BLACK] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AUGA3B，' + ItemTpl.ASSAULTRIFLE_STEYR_AUG_A3_556X45_ASSAULT_RIFLE_BLACK + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_AUG_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AUGA3B，' + ItemTpl.MAGAZINE_556X45_AUG_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AUG A3 Black extended magazine',
        shortName: 'Extended',
        description: 'AUG A3 Black extended magazine'
      },
      ch: {
        name: 'AUG A3 Black 扩容弹匣',
        shortName: '扩容',
        description: 'AUG A3 Black 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AUGA3B，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AUGA3B，Id：' + createResult.itemId);
}
