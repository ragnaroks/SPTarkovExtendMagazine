...
export default function addExtendMagazine_AVT40(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_TOKAREV_AVT40_762X54R_AUTOMATIC_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AVT40，' + ItemTpl.ASSAULTCARBINE_TOKAREV_AVT40_762X54R_AUTOMATIC_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X54R_AVT_15RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AVT40，' + ItemTpl.MAGAZINE_762X54R_AVT_15RND + ' not found');
    return;
  }
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AVT40，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AVT40，Id：' + createResult.itemId);
}
