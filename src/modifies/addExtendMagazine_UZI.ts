...
const baseId: string = '68c457795a5c5fecc953f500';
...
export default function addExtendMagazine_UZI(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_IWI_UZI_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZI，' + ItemTpl.SMG_IWI_UZI_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_UZI_32RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZI，' + ItemTpl.MAGAZINE_9X19_UZI_32RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'UZI extended magazine',
        shortName: 'Extended',
        description: 'UZI extended magazine'
      },
      ch: {
        name: 'UZI 扩容弹匣',
        shortName: '扩容',
        description: 'UZI 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZI，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_UZI，Id：' + createResult.itemId);
}
