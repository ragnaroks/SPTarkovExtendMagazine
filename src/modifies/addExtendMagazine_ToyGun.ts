...
const baseId: string = '68c45ae59af2bfbfc62c8a00';
...
export default function addExtendMagazine_ToyGun(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_20X1MM_TOY_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_ToyGun，' + ItemTpl.PISTOL_20X1MM_TOY_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_20X1MM_DRUM_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_ToyGun，' + ItemTpl.MAGAZINE_20X1MM_DRUM_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'ToyGun extended magazine',
        shortName: 'Extended',
        description: 'ToyGun extended magazine'
      },
      ch: {
        name: 'ToyGun 扩容弹匣',
        shortName: '扩容',
        description: 'ToyGun 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_ToyGun，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_ToyGun，Id：' + createResult.itemId);
}
