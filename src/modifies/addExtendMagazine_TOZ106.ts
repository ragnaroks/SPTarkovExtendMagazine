...
const baseId: string = '68c450beb20eab5505e3c200';
...
export default function addExtendMagazine_TOZ106(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_TOZ106_20GA_BOLTACTION] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TOZ106，' + ItemTpl.SHOTGUN_TOZ106_20GA_BOLTACTION + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_20G_SB3X5_5RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TOZ106，' + ItemTpl.MAGAZINE_20G_SB3X5_5RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'TOZ-106 extended magazine',
        shortName: 'Extended',
        description: 'TOZ-106 extended magazine'
      },
      ch: {
        name: 'TOZ-106 扩容弹匣',
        shortName: '扩容',
        description: 'TOZ-106 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TOZ106，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_TOZ106，Id：' + createResult.itemId);
}
