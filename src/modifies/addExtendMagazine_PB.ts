...
const baseId: string = '68c4601b538f85f57b959300';
...
export default function addExtendMagazine_PB(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_PB_9X18PM_SILENCED] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PB，' + ItemTpl.PISTOL_PB_9X18PM_SILENCED + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_PM_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PB，' + ItemTpl.MAGAZINE_9X18PM_PM_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PB extended magazine',
        shortName: 'Extended',
        description: 'PB extended magazine'
      },
      ch: {
        name: 'PB 扩容弹匣',
        shortName: '扩容',
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PB，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PB，Id：' + createResult.itemId);
}
