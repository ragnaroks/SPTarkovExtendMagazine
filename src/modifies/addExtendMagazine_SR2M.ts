...
const baseId: string = '68c4569ac5bbb09eaa411c00';
...
export default function addExtendMagazine_SR2M(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_SR2M_VERESK_9X21_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR2M，' + ItemTpl.SMG_SR2M_VERESK_9X21_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X21_SR2M_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR2M，' + ItemTpl.MAGAZINE_9X21_SR2M_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SR-2M extended magazine',
        shortName: 'Extended',
        description: 'SR-2M extended magazine'
      },
      ch: {
        name: 'SR-2M 扩容弹匣',
        shortName: '扩容',
        description: 'SR-2M 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR2M，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SR2M，Id：' + createResult.itemId);
}
