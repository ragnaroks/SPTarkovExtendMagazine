...
const baseId: string = '68c4241404ae359500581700';
...
export default function addExtendMagazine_AK12(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK12_545X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK12，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK12_545X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_AK12_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK12，' + ItemTpl.MAGAZINE_545X39_AK12_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AK-12 extended magazine',
        shortName: 'Extended',
        description: 'AK-12 extended magazine'
      },
      ch: {
        name: 'AK-12 扩容弹匣',
        shortName: '扩容',
        description: 'AK-12 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK12，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AK12，Id：' + createResult.itemId);
}
