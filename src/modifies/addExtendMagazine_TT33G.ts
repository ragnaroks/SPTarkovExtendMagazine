...
const baseId: string = '68c4621ca497bfb163ca6a00';
...
export default function addExtendMagazine_TT33G(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_TT33_762X25_TT_PISTOL_GOLDEN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33G，' + ItemTpl.PISTOL_TT33_762X25_TT_PISTOL_GOLDEN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X25TT_TT105_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33G，' + ItemTpl.MAGAZINE_762X25TT_TT105_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'TT-33 Golden extended magazine',
        shortName: 'Extended',
        description: 'TT-33 Golden extended magazine'
      },
      ch: {
        name: 'TT-33 Golden 扩容弹匣',
        shortName: '扩容',
        description: 'TT-33 Golden 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_TT33G，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_TT33G，Id：' + createResult.itemId);
}
