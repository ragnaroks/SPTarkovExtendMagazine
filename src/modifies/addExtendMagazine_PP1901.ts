...
const baseId: string = '68c455b052b1a904a6510900';
...
export default function addExtendMagazine_PP1901(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_PP1901_VITYAZ_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP1901，' + ItemTpl.SMG_PP1901_VITYAZ_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_PP1901_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP1901，' + ItemTpl.MAGAZINE_9X19_PP1901_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PP-19-01 Vityaz extended magazine',
        shortName: 'Extended',
        description: 'PP-19-01 Vityaz extended magazine'
      },
      ch: {
        name: 'PP-19-01 Vityaz 扩容弹匣',
        shortName: '扩容',
        description: 'PP-19-01 Vityaz 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP1901，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PP1901，Id：' + createResult.itemId);
}
