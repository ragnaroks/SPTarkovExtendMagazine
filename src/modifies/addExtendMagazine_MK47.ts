...
const baseId: string = '68c43390d02e90d2df900900';
...
export default function addExtendMagazine_MK47(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_CMMG_MK47_MUTANT_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MK47，' + ItemTpl.ASSAULTRIFLE_CMMG_MK47_MUTANT_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_GEN_M3_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MK47，' + ItemTpl.MAGAZINE_366TKM_GEN_M3_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MK47 extended magazine',
        shortName: 'Extended',
        description: 'MK47 extended magazine'
      },
      ch: {
        name: 'MK47 扩容弹匣',
        shortName: '扩容',
        description: 'MK47 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MK47，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MK47，Id：' + createResult.itemId);
}
