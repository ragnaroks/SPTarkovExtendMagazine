...
const baseId: string = '68c45c72ec4280c2a2fa0400';
...
export default function addExtendMagazine_DE50MKXIX(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_MK_XIX_50_AE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50MKXIX，' + ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_MK_XIX_50_AE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_127X33_DE_7RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50MKXIX，' + ItemTpl.MAGAZINE_127X33_DE_7RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'DE .50 MK XIX extended magazine',
        shortName: 'Extended',
        description: 'DE .50 MK XIX extended magazine'
      },
      ch: {
        name: 'DE .50 MK XIX 扩容弹匣',
        shortName: '扩容',
        description: 'DE .50 MK XIX 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50MKXIX，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_DE50MKXIX，Id：' + createResult.itemId);
}
