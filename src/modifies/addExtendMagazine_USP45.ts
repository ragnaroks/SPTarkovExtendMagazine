...
const baseId: string = '68c4625eee07393a27094900';
...
export default function addExtendMagazine_USP45(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_HK_USP_45_ACP] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_USP45，' + ItemTpl.PISTOL_HK_USP_45_ACP + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_45ACP_USP45_12RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_USP45，' + ItemTpl.MAGAZINE_45ACP_USP45_12RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'USP .45 extended magazine',
        shortName: 'Extended',
        description: 'USP .45 extended magazine'
      },
      ch: {
        name: 'USP .45 扩容弹匣',
        shortName: '扩容',
        description: 'USP .45 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_USP45，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_USP45，Id：' + createResult.itemId);
}
