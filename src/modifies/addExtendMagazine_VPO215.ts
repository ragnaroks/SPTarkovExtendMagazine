...
const baseId: string = '68c43c637f01b0bd3da85400';
...
export default function addExtendMagazine_VPO215(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_MOLOT_ARMS_VPO101_VEPRHUNTER_762X51_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO215，' + ItemTpl.ASSAULTCARBINE_MOLOT_ARMS_VPO101_VEPRHUNTER_762X51_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_VPO215_4RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO215，' + ItemTpl.MAGAZINE_366TKM_VPO215_4RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'VPO-215 extended magazine',
        shortName: 'Extended',
        description: 'VPO-215 extended magazine'
      },
      ch: {
        name: 'VPO-215 扩容弹匣',
        shortName: '扩容',
        description: 'VPO-215 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_VPO215，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_VPO215，Id：' + createResult.itemId);
}
