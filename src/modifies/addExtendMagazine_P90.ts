...
const baseId: string = '68c453e6b320868346ef2100';
...
export default function addExtendMagazine_P90(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_FN_P90_57X28_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P90，' + ItemTpl.SMG_FN_P90_57X28_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_57X28_P90_50RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P90，' + ItemTpl.MAGAZINE_57X28_P90_50RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'P90 extended magazine',
        shortName: 'Extended',
        description: 'P90 extended magazine'
      },
      ch: {
        name: 'P90 扩容弹匣',
        shortName: '扩容',
        description: 'P90 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_P90，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_P90，Id：' + createResult.itemId);
}
