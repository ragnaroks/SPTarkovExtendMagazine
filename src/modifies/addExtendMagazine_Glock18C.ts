...
const baseId: string = '68c46381b2d3f3421cd9a000';
...
export default function addExtendMagazine_Glock18C(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_GLOCK_18C_9X19_MACHINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock18C，' + ItemTpl.PISTOL_GLOCK_18C_9X19_MACHINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock18C，' + ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Glock 18C extended magazine',
        shortName: 'Extended',
        description: 'Glock 18C extended magazine'
      },
      ch: {
        name: 'Glock 18C 扩容弹匣',
        shortName: '扩容',
        description: 'Glock 18C 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock18C，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_Glock18C，Id：' + createResult.itemId);
}
