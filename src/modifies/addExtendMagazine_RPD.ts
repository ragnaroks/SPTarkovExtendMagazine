...
const baseId: string = '68c4483ab51978221dcc0600';
...
export default function addExtendMagazine_RPD(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_DEGTYAREV_RPD_762X39_MACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPD，' + ItemTpl.MACHINEGUN_DEGTYAREV_RPD_762X39_MACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X39_BUBEN_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPD，' + ItemTpl.MAGAZINE_762X39_BUBEN_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'RPD extended magazine',
        shortName: 'Extended',
        description: 'RPD extended magazine'
      },
      ch: {
        name: 'RPD 扩容弹匣',
        shortName: '扩容',
        description: 'RPD 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPD，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_RPD，Id：' + createResult.itemId);
}
