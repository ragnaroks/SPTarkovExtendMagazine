...
const baseId: string = '68c45df74b6f28b6153a6700';
...
export default function addExtendMagazine_Glock19X(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_GLOCK_19X_9X19] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock19X，' + ItemTpl.PISTOL_GLOCK_19X_9X19 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_G19X_19RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock19X，' + ItemTpl.MAGAZINE_9X19_G19X_19RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Glock 19X extended magazine',
        shortName: 'Extended',
        description: 'Glock 19X extended magazine'
      },
      ch: {
        name: 'Glock 19X 扩容弹匣',
        shortName: '扩容',
        description: 'Glock 19X 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock19X，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_Glock19X，Id：' + createResult.itemId);
}
