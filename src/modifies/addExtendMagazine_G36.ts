...
const baseId: string = '68c43081521eaf620c42b100';
...
export default function addExtendMagazine_G36(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_HK_G36_556X45_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_G36，' + ItemTpl.ASSAULTRIFLE_HK_G36_556X45_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_G36_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_G36，' + ItemTpl.MAGAZINE_556X45_G36_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'G36 extended magazine',
        shortName: 'Extended',
        description: 'G36 extended magazine'
      },
      ch: {
        name: 'G36 扩容弹匣',
        shortName: '扩容',
        description: 'G36 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_G36，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_G36，Id：' + createResult.itemId);
}
