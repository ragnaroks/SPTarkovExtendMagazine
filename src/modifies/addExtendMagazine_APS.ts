...
const baseId: string = '68c45a9a54e7eccbb0143f00';
...
export default function addExtendMagazine_APS(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_STECHKIN_APS_9X18PM_MACHINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APS，' + ItemTpl.PISTOL_STECHKIN_APS_9X18PM_MACHINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_APS_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APS，' + ItemTpl.MAGAZINE_9X18PM_APS_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'APS extended magazine',
        shortName: 'Extended',
        description: 'APS extended magazine'
      },
      ch: {
        name: 'APS 扩容弹匣',
        shortName: '扩容',
        description: 'APS 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APS，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_APS，Id：' + createResult.itemId);
}
