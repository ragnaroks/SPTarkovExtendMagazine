...
const baseId: string = '68c42a88b1089114e7671500';
...
export default function addExtendMagazine_AK105(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK104_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK105，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AK104_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_6L23_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK105，' + ItemTpl.MAGAZINE_545X39_6L23_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AK-105 extended magazine',
        shortName: 'Extended',
        description: 'AK-105 extended magazine'
      },
      ch: {
        name: 'AK-105 扩容弹匣',
        shortName: '扩容',
        description: 'AK-105 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK105，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AK105，Id：' + createResult.itemId);
}
