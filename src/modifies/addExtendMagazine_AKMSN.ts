...
const baseId: string = '68c42cce7985ee45ebcba200';
...
export default function addExtendMagazine_AKMSN(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKMSN_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMSN，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKMSN_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_AKMS_AL_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMSN，' + ItemTpl.MAGAZINE_366TKM_AKMS_AL_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AKMSN extended magazine',
        shortName: 'Extended',
        description: 'AKMSN extended magazine'
      },
      ch: {
        name: 'AKMSN 扩容弹匣',
        shortName: '扩容',
        description: 'AKMSN 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMSN，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AKMSN，Id：' + createResult.itemId);
}
