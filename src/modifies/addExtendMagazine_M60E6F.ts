...
const baseId: string = '68c4477a266bf74796d3ee00';
...
export default function addExtendMagazine_M60E6F(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_US_ORDNANCE_M60E6_762X51_LIGHT_MACHINE_GUN_FDE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6F，' + ItemTpl.MACHINEGUN_US_ORDNANCE_M60E6_762X51_LIGHT_MACHINE_GUN_FDE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6F，' + ItemTpl.MAGAZINE_762X51_ASSAULT_BOX_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M60E6 FDE extended magazine',
        shortName: 'Extended',
        description: 'M60E6 FDE extended magazine'
      },
      ch: {
        name: 'M60E6 FDE 扩容弹匣',
        shortName: '扩容',
        description: 'M60E6 FDE 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6F，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M60E6F，Id：' + createResult.itemId);
}
