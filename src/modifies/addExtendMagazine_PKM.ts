...
const baseId: string = '68c447bddcb7c1aaa5d00300';
...
export default function addExtendMagazine_PKM(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_KALASHNIKOV_PKM_762X54R_MACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PKM，' + ItemTpl.MACHINEGUN_KALASHNIKOV_PKM_762X54R_MACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X54R_PK_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PKM，' + ItemTpl.MAGAZINE_762X54R_PK_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        ...
        shortName: 'Extended',
        description: 'PKM extended magazine'
      },
      ch: {
        name: 'PKM 扩容弹匣',
        shortName: '扩容',
        description: 'PKM 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PKM，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PKM，Id：' + createResult.itemId);
}
