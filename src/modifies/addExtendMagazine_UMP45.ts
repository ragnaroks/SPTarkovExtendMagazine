...
const baseId: string = '68c4572ba5888089300be400';
...
export default function addExtendMagazine_UMP45(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_HK_UMP_45_ACP_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UMP45，' + ItemTpl.SMG_HK_UMP_45_ACP_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_45ACP_UMP_45_25RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UMP45，' + ItemTpl.MAGAZINE_45ACP_UMP_45_25RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        ...
        shortName: 'Extended',
        description: 'UMP-45 extended magazine'
      },
      ch: {
        name: 'UMP-45 扩容弹匣',
        shortName: '扩容',
        description: 'UMP-45 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UMP45，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_UMP45，Id：' + createResult.itemId);
}
