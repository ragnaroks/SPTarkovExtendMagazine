...
const baseId: string = '68c42b84445bd5932ba48600';
...
export default function addExtendMagazine_AKM(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKM_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKM，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKM_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_AK55_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKM，' + ItemTpl.MAGAZINE_366TKM_AK55_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        ...
        shortName: 'Extended',
        description: 'AKM extended magazine'
      },
      ch: {
        name: 'AKM 扩容弹匣',
        shortName: '扩容',
        description: 'AKM 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKM，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AKM，Id：' + createResult.itemId);
}
