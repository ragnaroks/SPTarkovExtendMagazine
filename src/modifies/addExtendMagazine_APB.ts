...
const baseId: string = '68c45a4e33a344fb5ab74000';
...
export default function addExtendMagazine_APB(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_STECHKIN_APB_9X18PM_SILENCED_MACHINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APB，' + ItemTpl.PISTOL_STECHKIN_APB_9X18PM_SILENCED_MACHINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_APS_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APB，' + ItemTpl.MAGAZINE_9X18PM_APS_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'APB extended magazine',
        shortName: 'Extended',
        description: 'APB extended magazine'
      },
      ch: {
        name: 'APB 扩容弹匣',
        shortName: '扩容',
        description: 'APB 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_APB，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_APB，Id：' + createResult.itemId);
}
