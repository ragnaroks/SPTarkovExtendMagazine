...
const baseId: string = '68c46120672e0b4f2f65c500';
...
export default function addExtendMagazine_PM(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_MAKAROV_PM_9X18PM] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PM，' + ItemTpl.PISTOL_MAKAROV_PM_9X18PM + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_PM_8RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PM，' + ItemTpl.MAGAZINE_9X18PM_PM_8RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PM extended magazine',
        shortName: 'Extended',
        description: 'PM extended magazine'
      },
      ch: {
        name: 'PM 扩容弹匣',
        shortName: '扩容',
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PM，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PM，Id：' + createResult.itemId);
}
