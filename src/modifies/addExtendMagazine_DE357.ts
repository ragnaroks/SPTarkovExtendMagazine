...
const baseId: string = '68c45b4e32f8fc977a390800';
...
export default function addExtendMagazine_DE357(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_L5_357] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE357，' + ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_L5_357 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X33R_DE_9RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE357，' + ItemTpl.MAGAZINE_9X33R_DE_9RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'DE .357 extended magazine',
        shortName: 'Extended',
        description: 'DE .357 extended magazine'
      },
      ch: {
        name: 'DE .357 扩容弹匣',
        shortName: '扩容',
        description: 'DE .357 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE357，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_DE357，Id：' + createResult.itemId);
}
