...
const baseId: string = '68c4487ccd34b02e20458300';
...
export default function addExtendMagazine_RPDN(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.MACHINEGUN_DEGTYAREV_RPDN_762X39_MACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPDN，' + ItemTpl.MACHINEGUN_DEGTYAREV_RPDN_762X39_MACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X39_BUBEN_100RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPDN，' + ItemTpl.MAGAZINE_762X39_BUBEN_100RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'RPDN extended magazine',
        shortName: 'Extended',
        description: 'RPDN extended magazine'
      },
      ch: {
        name: 'RPDN 扩容弹匣',
        shortName: '扩容',
        description: 'RPDN 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_RPDN，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_RPDN，Id：' + createResult.itemId);
}
