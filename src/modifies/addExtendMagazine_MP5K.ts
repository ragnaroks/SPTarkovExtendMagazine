...
const baseId: string = '68c451b5af2b42e2dd316000';
...
export default function addExtendMagazine_MP5K(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_HK_MP5K_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5K，' + ItemTpl.SMG_HK_MP5K_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MP5_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5K，' + ItemTpl.MAGAZINE_9X19_MP5_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP5K extended magazine',
        shortName: 'Extended',
        description: 'MP5K extended magazine'
      },
      ch: {
        name: 'MP5K 扩容弹匣',
        shortName: '扩容',
        description: 'MP5K 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP5K，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP5K，Id：' + createResult.itemId);
}
