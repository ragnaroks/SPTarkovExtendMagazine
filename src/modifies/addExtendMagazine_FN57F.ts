...
export default function addExtendMagazine_FN57F(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  ...
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_FN57F，' + ItemTpl.MAGAZINE_57X28_57_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'FN57 FDE extended magazine',
        shortName: 'Extended',
        description: 'FN57 FDE extended magazine'
      },
      ch: {
        name: 'FN57 FDE 扩容弹匣',
        shortName: '扩容',
        description: 'FN57 FDE 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_FN57F，Id：' + createResult.itemId);
}
