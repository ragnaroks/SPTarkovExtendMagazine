...
const baseId: string = '68c4534cef850986d7adf900';
...
export default function addExtendMagazine_MPX(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_SIG_MPX_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MPX，' + ItemTpl.SMG_SIG_MPX_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MPX_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MPX，' + ItemTpl.MAGAZINE_9X19_MPX_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MPX extended magazine',
        shortName: 'Extended',
        description: 'MPX extended magazine'
      },
      ch: {
        name: 'MPX 扩容弹匣',
        shortName: '扩容',
        description: 'MPX 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MPX，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MPX，Id：' + createResult.itemId);
}
