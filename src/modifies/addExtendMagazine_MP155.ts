...
const baseId: string = '68c44ef4ba3141bf55122d00';
...
export default function addExtendMagazine_MP155(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_MP155_12GA_SEMIAUTOMATIC] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP155，' + ItemTpl.SHOTGUN_MP155_12GA_SEMIAUTOMATIC + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_12G_MP153X6_6RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP155，' + ItemTpl.MAGAZINE_12G_MP153X6_6RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP-155 extended magazine',
        shortName: 'Extended',
        description: 'MP-155 extended magazine'
      },
      ch: {
        name: 'MP-155 扩容弹匣',
        shortName: '扩容',
        description: 'MP-155 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP155，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP155，Id：' + createResult.itemId);
}
