...
const baseId: string = '68c45cb861a95c1f37dc0f00';
...
export default function addExtendMagazine_FN57(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_FN_FIVESEVEN_MK2_57X28] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_FN57，' + ItemTpl.PISTOL_FN_FIVESEVEN_MK2_57X28 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_57X28_57_20RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_FN57，' + ItemTpl.MAGAZINE_57X28_57_20RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'FN57 extended magazine',
        shortName: 'Extended',
        description: 'FN57 extended magazine'
      },
      ch: {
        name: 'FN57 扩容弹匣',
        shortName: '扩容',
        description: 'FN57 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_FN57，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_FN57，Id：' + createResult.itemId);
}
