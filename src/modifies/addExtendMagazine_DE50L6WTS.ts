...
const baseId: string = '68c45c33b34784741cb0fe00';
...
export default function addExtendMagazine_DE50L6WTS(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_L6_50_AE_PISTOL_WTS] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L6WTS，' + ItemTpl.PISTOL_MAGNUM_RESEARCH_DESERT_EAGLE_L6_50_AE_PISTOL_WTS + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_127X33_DE_7RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L6WTS，' + ItemTpl.MAGAZINE_127X33_DE_7RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'DE .50 L6 WTS extended magazine',
        shortName: 'Extended',
        description: 'DE .50 L6 WTS extended magazine'
      },
      ch: {
        name: 'DE .50 L6 WTS 扩容弹匣',
        shortName: '扩容',
        description: 'DE .50 L6 WTS 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L6WTS，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L6WTS，Id：' + createResult.itemId);
}
