...
const baseId: string = '68c45820b6cd52dd66b9c000';
...
export default function addExtendMagazine_UZIPP(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_IWI_UZI_PRO_PISTOL_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPP，' + ItemTpl.SMG_IWI_UZI_PRO_PISTOL_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_UZI_PRO_32RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPP，' + ItemTpl.MAGAZINE_9X19_UZI_PRO_32RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'UZI PRO Pistol extended magazine',
        shortName: 'Extended',
        description: 'UZI PRO Pistol extended magazine'
      },
      ch: {
        name: 'UZI PRO Pistol 扩容弹匣',
        shortName: '扩容',
        description: 'UZI PRO Pistol 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPP，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_UZIPP，Id：' + createResult.itemId);
}
