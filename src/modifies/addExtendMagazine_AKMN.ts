...
const baseId: string = '68c42b99e755772f301e7500';
...
export default function addExtendMagazine_AKMN(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKMN_762X39_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMN，' + ItemTpl.ASSAULTRIFLE_KALASHNIKOV_AKMN_762X39_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_6L10_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMN，' + ItemTpl.MAGAZINE_366TKM_6L10_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AKMN extended magazine',
        shortName: 'Extended',
        description: 'AKMN extended magazine'
      },
      ch: {
        name: 'AKMN 扩容弹匣',
        shortName: '扩容',
        description: 'AKMN 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AKMN，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AKMN，Id：' + createResult.itemId);
}
