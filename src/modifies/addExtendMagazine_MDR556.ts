...
const baseId: string = '68c430169ceccffa216dc600';
...
export default function addExtendMagazine_MDR556(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_DESERT_TECH_MDR_556X45_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MDR556，' + ItemTpl.ASSAULTRIFLE_DESERT_TECH_MDR_556X45_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_GEN_M3_WINDOW_FDE_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MDR556，' + ItemTpl.MAGAZINE_556X45_GEN_M3_WINDOW_FDE_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MDR extended magazine',
        shortName: 'Extended',
        description: 'MDR extended magazine'
      },
      ch: {
        name: 'MDR 扩容弹匣',
        shortName: '扩容',
        description: 'MDR 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MDR556，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MDR556，Id：' + createResult.itemId);
}
