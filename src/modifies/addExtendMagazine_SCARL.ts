...
const baseId: string = '68c43584c65e7961cca1ad00';
...
export default function addExtendMagazine_SCARL(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_FN_SCARL_556X45_ASSAULT_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARL，' + ItemTpl.ASSAULTRIFLE_FN_SCARL_556X45_ASSAULT_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_MK16_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARL，' + ItemTpl.MAGAZINE_556X45_MK16_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SCAR-L extended magazine',
        shortName: 'Extended',
        description: 'SCAR-L extended magazine'
      },
      ch: {
        name: 'SCAR-L 扩容弹匣',
        shortName: '扩容',
        description: 'SCAR-L 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARL，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SCARL，Id：' + createResult.itemId);
}
