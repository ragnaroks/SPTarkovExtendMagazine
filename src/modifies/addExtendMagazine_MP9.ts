...
const baseId: string = '68c452a7ed9f2c93578a9600';
...
export default function addExtendMagazine_MP9(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_BT_MP9_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9，' + ItemTpl.SMG_BT_MP9_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MP9_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9，' + ItemTpl.MAGAZINE_9X19_MP9_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP9 extended magazine',
        shortName: 'Extended',
        description: 'MP9 extended magazine'
      },
      ch: {
        name: 'MP9 扩容弹匣',
        shortName: '扩容',
        description: 'MP9 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP9，Id：' + createResult.itemId);
}
