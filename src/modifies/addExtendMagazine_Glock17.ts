...
const baseId: string = '68c45d3e39536931c4e52b00';
...
export default function addExtendMagazine_Glock17(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_GLOCK_17_9X19] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock17，' + ItemTpl.PISTOL_GLOCK_17_9X19 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock17，' + ItemTpl.MAGAZINE_9X19_GLOCK_9X19_17RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Glock 17 extended magazine',
        shortName: 'Extended',
        description: 'Glock 17 extended magazine'
      },
      ch: {
        name: 'Glock 17 扩容弹匣',
        shortName: '扩容',
        description: 'Glock 17 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Glock17，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_Glock17，Id：' + createResult.itemId);
}
