...
const baseId: string = '68c4552b453386502cda6900';
...
export default function addExtendMagazine_PP9101(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_PP9101_KEDRB_9X18PM_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9101，' + ItemTpl.SMG_PP9101_KEDRB_9X18PM_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X18PM_PP91_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9101，' + ItemTpl.MAGAZINE_9X18PM_PP91_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PP-91-01 Kedr-B extended magazine',
        shortName: 'Extended',
        description: 'PP-91-01 Kedr-B extended magazine'
      },
      ch: {
        name: 'PP-91-01 Kedr-B 扩容弹匣',
        shortName: '扩容',
        description: 'PP-91-01 Kedr-B 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PP9101，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PP9101，Id：' + createResult.itemId);
}
