...
const baseId: string = '68c461683d98103df65bd000';
...
export default function addExtendMagazine_SR1MP(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_SERDYUKOV_SR1MP_GYURZA_9X21] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR1MP，' + ItemTpl.PISTOL_SERDYUKOV_SR1MP_GYURZA_9X21 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X21_SR1MP_18RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR1MP，' + ItemTpl.MAGAZINE_9X21_SR1MP_18RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SR-1MP extended magazine',
        shortName: 'Extended',
        description: 'SR-1MP extended magazine'
      },
      ch: {
        name: 'SR-1MP 扩容弹匣',
        shortName: '扩容',
        description: 'SR-1MP 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SR1MP，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SR1MP，Id：' + createResult.itemId);
}
