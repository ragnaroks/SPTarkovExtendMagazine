...
const baseId: string = '68c453178296901e76a4f400';
...
export default function addExtendMagazine_MP9N(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_BT_MP9N_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9N，' + ItemTpl.SMG_BT_MP9N_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_MP9_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9N，' + ItemTpl.MAGAZINE_9X19_MP9_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP9-N extended magazine',
        shortName: 'Extended',
        description: 'MP9-N extended magazine'
      },
      ch: {
        name: 'MP9-N 扩容弹匣',
        shortName: '扩容',
        description: 'MP9-N 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP9N，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP9N，Id：' + createResult.itemId);
}
