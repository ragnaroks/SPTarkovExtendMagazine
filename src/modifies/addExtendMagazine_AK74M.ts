...
const baseId: string = '68c424c1c717db5865a50b00';
...
export default function addExtendMagazine_AK74M(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  ...
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK74M，' + ItemTpl.MAGAZINE_545X39_6L23_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        ...
        shortName: 'Extended',
        description: 'AK-74M extended magazine'
      },
      ch: {
        name: 'AK-74M 扩容弹匣',
        shortName: '扩容',
        description: 'AK-74M 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_AK74M，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_AK74M，Id：' + createResult.itemId);
}
