...
const baseId: string = '68c45e4d52299a1ce16afd00';
...
export default function addExtendMagazine_M9A3(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_BERETTA_M9A3_9X19] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M9A3，' + ItemTpl.PISTOL_BERETTA_M9A3_9X19 + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_M9A3_17RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M9A3，' + ItemTpl.MAGAZINE_9X19_M9A3_17RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M9A3 extended magazine',
        shortName: 'Extended',
        description: 'M9A3 extended magazine'
      },
      ch: {
        name: 'M9A3 扩容弹匣',
        shortName: '扩容',
        description: 'M9A3 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M9A3，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M9A3，Id：' + createResult.itemId);
}
