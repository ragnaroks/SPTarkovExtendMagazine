...
const baseId: string = '68c455d953212697b6a42c00';
...
export default function addExtendMagazine_PPSH41(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_PPSH41_762X25_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PPSH41，' + ItemTpl.SMG_PPSH41_762X25_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X25TT_PPSH_35RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PPSH41，' + ItemTpl.MAGAZINE_762X25TT_PPSH_35RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'PPSh-41 extended magazine',
        shortName: 'Extended',
        description: 'PPSh-41 extended magazine'
      },
      ch: {
        name: 'PPSh-41 扩容弹匣',
        shortName: '扩容',
        description: 'PPSh-41 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_PPSH41，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_PPSH41，Id：' + createResult.itemId);
}
