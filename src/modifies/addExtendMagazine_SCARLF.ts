...
const baseId: string = '68c435cbdeb845d79dc15600';
...
export default function addExtendMagazine_SCARLF(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_FN_SCARL_556X45_ASSAULT_RIFLE_FDE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARLF，' + ItemTpl.ASSAULTRIFLE_FN_SCARL_556X45_ASSAULT_RIFLE_FDE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_556X45_MK16_FDE_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARLF，' + ItemTpl.MAGAZINE_556X45_MK16_FDE_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SCAR-L FDE extended magazine',
        shortName: 'Extended',
        description: 'SCAR-L FDE extended magazine'
      },
      ch: {
        name: 'SCAR-L FDE 扩容弹匣',
        shortName: '扩容',
        description: 'SCAR-L FDE 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SCARLF，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SCARLF，Id：' + createResult.itemId);
}
