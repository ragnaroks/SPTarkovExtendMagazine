...
const baseId: string = '68c45ea09b5b85a6137e1000';
...
export default function addExtendMagazine_M45A1(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.PISTOL_COLT_M45A1_45_ACP] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M45A1，' + ItemTpl.PISTOL_COLT_M45A1_45_ACP + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_45ACP_WILSON_7RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M45A1，' + ItemTpl.MAGAZINE_45ACP_WILSON_7RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'M45A1 extended magazine',
        shortName: 'Extended',
        description: 'M45A1 extended magazine'
      },
      ch: {
        name: 'M45A1 扩容弹匣',
        shortName: '扩容',
        description: 'M45A1 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_M45A1，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_M45A1，Id：' + createResult.itemId);
}
