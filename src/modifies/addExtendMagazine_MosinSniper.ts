...
const baseId: string = '68c43b3902ece73e1f0ecf00';
...
export default function addExtendMagazine_MosinSniper(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SNIPERRIFLE_MOSIN_762X54R_BOLTACTION_RIFLE_SNIPER] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MosinSniper，' + ItemTpl.SNIPERRIFLE_MOSIN_762X54R_BOLTACTION_RIFLE_SNIPER + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X54R_AA762R_02_10RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MosinSniper，' + ItemTpl.MAGAZINE_762X54R_AA762R_02_10RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Mosin Sniper extended magazine',
        shortName: 'Extended',
        description: 'Mosin Sniper extended magazine'
      },
      ch: {
        name: 'Mosin Sniper 扩容弹匣',
        shortName: '扩容',
        description: 'Mosin Sniper 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MosinSniper，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MosinSniper，Id：' + createResult.itemId);
}
