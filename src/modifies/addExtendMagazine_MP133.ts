...
const baseId: string = '68c44ddc030ba33b42a2ed00';
...
export default function addExtendMagazine_MP133(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SHOTGUN_MP133_12GA_PUMPACTION] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP133，' + ItemTpl.SHOTGUN_MP133_12GA_PUMPACTION + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_12G_MP133X6_6RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP133，' + ItemTpl.MAGAZINE_12G_MP133X6_6RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'MP-133 extended magazine',
        shortName: 'Extended',
        description: 'MP-133 extended magazine'
      },
      ch: {
        name: 'MP-133 扩容弹匣',
        shortName: '扩容',
        description: 'MP-133 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_MP133，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_MP133，Id：' + createResult.itemId);
}
