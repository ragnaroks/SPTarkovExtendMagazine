...
const baseId: string = '68c41d8139eea1f37f919900';
...
export default function addExtendMagazine_SAGAK545S(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTRIFLE_SAG_AK545_SHORT_545X39_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAGAK545S，' + ItemTpl.ASSAULTRIFLE_SAG_AK545_SHORT_545X39_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_545X39_AK12_30RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAGAK545S，' + ItemTpl.MAGAZINE_545X39_AK12_30RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'AK-545 Short extended magazine',
        shortName: 'Extended',
        description: 'AK-545 Short extended magazine'
      },
      ch: {
        name: 'AK-545 Short 扩容弹匣',
        shortName: '扩容',
        description: 'AK-545 Short 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SAGAK545S，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SAGAK545S，Id：' + createResult.itemId);
}
