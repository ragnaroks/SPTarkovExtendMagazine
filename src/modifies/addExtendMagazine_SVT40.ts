...
export default function addExtendMagazine_SVT40(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_TOKAREV_SVT40_762X54R_RIFLE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SVT40，' + ItemTpl.ASSAULTCARBINE_TOKAREV_SVT40_762X54R_RIFLE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_762X54R_AVT_15RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SVT40，' + ItemTpl.MAGAZINE_762X54R_AVT_15RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'SVT-40 extended magazine',
        shortName: 'Extended',
        description: 'SVT-40 extended magazine'
      },
      ch: {
        name: 'SVT-40 扩容弹匣',
        shortName: '扩容',
        description: 'SVT-40 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SVT40，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SVT40，Id：' + createResult.itemId);
}
