...
const baseId: string = '68c45ba968e9b045c06c7d00';
...
export default function addExtendMagazine_DE50L5(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  ...
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L5，' + ItemTpl.MAGAZINE_127X33_DE_7RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'DE .50 L5 extended magazine',
        shortName: 'Extended',
        description: 'DE .50 L5 extended magazine'
      },
      ch: {
        name: 'DE .50 L5 扩容弹匣',
        shortName: '扩容',
        description: 'DE .50 L5 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L5，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_DE50L5，Id：' + createResult.itemId);
}
