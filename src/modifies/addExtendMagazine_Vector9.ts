...
const baseId: string = '68c45946bd33f11866f0bd00';
...
export default function addExtendMagazine_Vector9(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.SMG_TDI_KRISS_VECTOR_GEN2_9X19_SUBMACHINE_GUN] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector9，' + ItemTpl.SMG_TDI_KRISS_VECTOR_GEN2_9X19_SUBMACHINE_GUN + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_9X19_BIG_STICK_31RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector9，' + ItemTpl.MAGAZINE_9X19_BIG_STICK_31RND + ' not found');
    return;
  }
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'Vector 9x19 extended magazine',
        shortName: 'Extended',
        description: 'Vector 9x19 extended magazine'
      },
      ch: {
        name: 'Vector 9x19 扩容弹匣',
        shortName: '扩容',
        description: 'Vector 9x19 扩容弹匣'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_Vector9，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_Vector9，Id：' + createResult.itemId);
}
