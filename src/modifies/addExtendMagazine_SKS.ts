...
const baseId: string = '68c41e03e6a63871179ff500';
...
export default function addExtendMagazine_SKS(logger: ILogger,modConfig:ModConfig,customItemService: CustomItemService,tables: IDatabaseTables,botConfig:IBotConfig) {
  const weapon = tables.templates.items[ItemTpl.ASSAULTCARBINE_TOZ_SIMONOV_SKS_762X39_CARBINE] || null;
  if(!weapon) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SKS，' + ItemTpl.ASSAULTCARBINE_TOZ_SIMONOV_SKS_762X39_CARBINE + ' not found');
    return;
  }

  const magazine = tables.templates.items[ItemTpl.MAGAZINE_366TKM_SKS_AALVX_35RND] || null;
  if(!magazine) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SKS，' + ItemTpl.MAGAZINE_366TKM_SKS_AALVX_35RND + ' not found');
    return;
  }
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovExtendMagazine]：addExtendMagazine_SKS，' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovExtendMagazine]：addExtendMagazine_SKS，Id：' + createResult.itemId);
}
